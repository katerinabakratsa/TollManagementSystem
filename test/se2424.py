...
def login(username, passw):
    ...
    if response and "token" in response:
        ...
        with open(".token", "w") as f:
            f.write(TOKEN)  # Αποθήκευση του token σε αρχείο
        print(f"Login successful. Token saved: {TOKEN}")
    else:
        print("Login failed.")
...
def reset_passes():
    ...
    try:
        with open(".token", "r") as f:
            TOKEN = f.read().strip()  # Διαβάζει το αποθηκευμένο token
    except FileNotFoundError:
        print("Error: No authentication token found. Please login first.")
        return

    headers = {"X-OBSERVATORY-AUTH": TOKEN}
    response = call_api("admin/resetpasses", "POST", headers=headers)
...
def charges_by(opid, from_date, to_date):
    endpoint = f"chargesBy/tollOpID/{opid}/date_from/{from_date}/date_to/{to_date}"
    data = call_api(endpoint, "GET")
    if data:
        print(json.dumps(data, indent=4))
